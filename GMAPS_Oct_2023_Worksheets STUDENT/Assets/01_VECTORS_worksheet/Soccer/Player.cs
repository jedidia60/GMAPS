...
public class Player : MonoBehaviour
{
    ...
    float AngleToPlayer()
    {
        ...
        Vector3 vectorA = OtherPlayer.transform.position - transform.position;
        DebugExtension.DebugArrow(transform.position, vectorA, Color.red);
        ...
        Vector3 vectorB = transform.forward - transform.position;
        DebugExtension.DebugArrow(transform.position, vectorB, Color.blue);

        // CALCULATING THE ANGLE

        float angle = Mathf.Acos(Dot(vectorA, vectorB) / (Magnitude(vectorA) * Magnitude(vectorB)));
